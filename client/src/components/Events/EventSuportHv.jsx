import {
  ...
  import { eventsSuportHv, distinctNameSamsungCount } from "../../api/events";
  ...
  const EventSuportHv = () => {
    ...
    const fetchEventsData = async () => {
      setIsLoading(true);
      try {
        const data = await eventsSuportHv();
        setEventsData(data);
      } catch (error) {
    ...
    useEffect(() => {
      const intervalId = setInterval(fetchEventsData, 1000);
  ...
  EventSuportHv.propTypes = {
    data: PropsTypes.array,
  };
  
  export default EventSuportHv;
